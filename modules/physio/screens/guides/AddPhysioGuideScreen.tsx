...
export default function AddPhysioGuideScreen() {
  const isWeb = Platform.OS === "web";
  const { theme } = useTheme();
  const { user } = useUserStore();
  const { addGuide, physioGuides } = usePhysioGuideStore();
  ...
  const uploadImage = async () => {
    ...
    const index =
      physioGuides.filter((guide) => guide.user_id === user.id).length + 1;
    const filePath = `Guides/Physio/${user.id}-GuideID-${index}`;
