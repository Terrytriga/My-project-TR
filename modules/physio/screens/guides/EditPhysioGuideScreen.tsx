...
export default function EditPhysioGuideScreen() {
  const isWeb = Platform.OS === "web";
  const navigation =
    useNavigation<NativeStackNavigationProp<PhysioGuideStack>>();
  const route = useRoute<RouteProp<PhysioGuideStack, "EditPhysioGuides">>();
  const id = route.params.id;
  const { updateGuide, physioGuides } = usePhysioGuideStore();
  const guide = physioGuides.find((findGuide) => findGuide.id === id);
  ...
  const uploadImage = async () => {
    ...
    const filePath = `Guides/Physio/${user.id}-GuideID-${index + 1}`;
