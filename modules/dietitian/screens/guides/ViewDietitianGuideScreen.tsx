...
const aspect = 16 / 9;
export default function ViewDietitianGuideScreen() {
  const isWeb = Platform.OS === "web";
  const { user } = useUserStore();
  const [isLoading, setIsLoading] = useState<boolean>(false);
  const { theme } = useTheme();
  const { dietitianGuides, deleteGuide } = useDietitianGuideStore();
  const navigation =
    useNavigation<NativeStackNavigationProp<DietitianGuideStack>>();
  const route =
    useRoute<RouteProp<DietitianGuideStack, "ViewDietitianGuide">>();
  const id = route.params.id;
  const guide = dietitianGuides.find((guide) => guide.id === id);

  function navigationEditMealHandler() {
    navigation.navigate("EditDietitianGuide", { id });
  }
