...
import { TrackingGoalWorkout } from "../../../utils/Types";
...
export default function SetWorkoutGoalScreen() {
  ...
  const {
    trackingGoalWorkout,
    addTrackingGoalWorkout,
    deleteTrackingGoalWorkout,
  } = useTrackingStore();
  ...
  async function workoutGoal() {
    if (!user) return;

    const isOverlapping = trackingGoalWorkout.some((goal) => {
    ...
    addTrackingGoalWorkout(data);
    navigation.navigate("Home");
  }

  async function deleteWorkoutGoal(workoutGoal: TrackingGoalWorkout) {
    if (trackingGoalWorkout.length === 0) return;
    const { error } = await supabase
      .from("trackinggoalworkout")
      .delete()
      .eq("id", workoutGoal.id);
    if (error) {
      TrackingAlert("Error", error.message);
      return;
    }
    deleteTrackingGoalWorkout(workoutGoal);
  }
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>How many workouts do you want to do?</Text>
      {Platform.OS !== "web" ? (
        <Pressable
      ...
      <Input
        ...
        onUpdateValue={(value: number) => setNumberOfWorkouts(value)}
        isValid={true}
          ...
          trackingGoalWorkout.map((goal) => (
            <View key={goal.id} style={styles.goalContainer}>
              ...
              <Pressable
                ...
                onPress={() => deleteWorkoutGoal(goal)}
              >
      ...
      <Button onPress={workoutGoal}>Add Goal</Button>
    </View>
  );
}
