...
export default function SetCaloryGoalScreen() {
  ...
  const {
    trackingGoalCalory,
    addTrackingGoalCalory,
    deleteTrackingGoalCalory,
  } = useTrackingStore();
  ...
  async function caloryGoal() {
    if (!user) return;

    const isOverlapping = trackingGoalCalory.some((goal) => {
    ...
    addTrackingGoalCalory(data);
    navigation.navigate("Home");
  }

  async function deleteCaloryGoal(caloryGoal: TrackingGoalCalory) {
    if (trackingGoalCalory.length === 0) return;
    ...
    deleteTrackingGoalCalory(caloryGoal);
  }
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>What is your calory budget?</Text>
      {Platform.OS !== "web" ? (
        <Pressable
      ...
      <Input
        ...
        onUpdateValue={(value: number) => setNumberOfCalories(value)}
        isValid={true}
      ...
      <Button onPress={caloryGoal}>Add Goal</Button>
    </View>
  );
}
